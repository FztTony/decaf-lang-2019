...
public class LLParser extends Phase<InputStream, Tree.TopLevel> {
    ...
    private class Parser extends decaf.frontend.parsing.LLTable {
        ...
        private SemValue parseSymbol(int symbol, Set<Integer> follow) {
            var result = query(symbol, token); // get production by token symbol
            Set<Integer> bgSet = beginSet(symbol);
            Set<Integer> edSet = followSet(symbol);
            edSet.addAll(follow);
            if (!bgSet.contains(token)) {
                hasErr = true;
            ...
            for (var i = 0; i < length; i++) { // parse right-hand side symbols one by one
                ...
                params[i + 1] = isNonTerminal(term)
                        ? parseSymbol(term, edSet) // for non terminals: recursively parse it
                        : matchToken(term) // for terminals: match token
                ;
            }

            if (!hasErr) act(actionId, params); // do user-defined action
            return params[0];
        }

        /**
         * Match if the token token is the same as the expected token.
         *
