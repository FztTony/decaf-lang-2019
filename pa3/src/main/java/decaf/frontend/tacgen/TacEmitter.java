...
public interface TacEmitter extends Visitor<FuncVisitor> {
    ...
    default void visitBinary(Tree.Binary expr, FuncVisitor mv) {
        ...
        if (expr.op.equals(Tree.BinaryOp.DIV) || expr.op.equals(Tree.BinaryOp.MOD)) {
            ...
            mv.visitPrint(RuntimeError.DIVISION_BY_ZERO_ERROR);
    ...
    default void visitVarSel(Tree.VarSel expr, FuncVisitor mv) {
        ...
        if (expr.isArrayLength) {
            var tmp = mv.visitIntrinsicCall(Intrinsic.ALLOCATE, true, eight);
            mv.visitStoreTo(tmp, three);
            var object = expr.receiver.get();
            object.accept(this, mv);
            mv.visitStoreTo(tmp, 4, object.val);
            expr.val = tmp;
            return;
        }
        if (expr.isMethodName) {
            if (expr.isStatic) {
                var tmp = mv.visitIntrinsicCall(Intrinsic.ALLOCATE, true, eight);
                mv.visitStoreTo(tmp, zero);
                mv.visitStoreTo(tmp, 4, mv.visitFuncEntry(expr.calleeName, expr.name));
                expr.val = tmp;
            } else {
                var tmp = mv.visitIntrinsicCall(Intrinsic.ALLOCATE, true, twelve);
                mv.visitStoreTo(tmp, one);
                var object = expr.receiver.get();
                object.accept(this, mv);
                var entry = mv.visitFuncEntry(object.val, expr.calleeName, expr.name);
                mv.visitStoreTo(tmp, 4, entry);
                mv.visitStoreTo(tmp, 8, object.val);
                expr.val = tmp;
            }
            return;
        }
    ...
    default void visitCall(Tree.Call expr, FuncVisitor mv) {
        ...
        var type = mv.visitLoadFrom(expr.callee.val);
        var entry = mv.visitLoadFrom(expr.callee.val, 4);
        var exit = mv.freshLabel();
        var one = mv.visitLoad(-1);
        ...
        mv.visitBranch(TacInstr.CondBranch.Op.BNEZ, mv.visitBinary(TacInstr.Binary.Op.ADD, type, one), notNonStatic);
        //1Non-Static
        mv.visitParm(mv.visitLoadFrom(expr.callee.val, 8));
        if (expr.type.isVoidType()) {
            mv.visitCall(entry, temps, false);
        } else {
            mv.visitAssign(expr.val, mv.visitCall(entry, temps, true));
        ...
        mv.visitBranch(TacInstr.CondBranch.Op.BNEZ, mv.visitBinary(TacInstr.Binary.Op.ADD, type, two), notLambda);
        //2Lambda
        var numCaptured = mv.visitLoadFrom(expr.callee.val, 8);
        var currentArgPos = mv.visitBinary(TacInstr.Binary.Op.ADD, expr.callee.val, mv.visitLoad(12));
        Function<FuncVisitor, Temp> test = v -> numCaptured;
        Consumer<FuncVisitor> body = v -> {
            var currentArg = v.visitLoadFrom(currentArgPos);
            v.visitParm(currentArg);
            v.visitBinarySelf(TacInstr.Binary.Op.ADD, numCaptured, one);
            v.visitBinarySelf(TacInstr.Binary.Op.ADD, currentArgPos, mv.visitLoad(4));
        };
    ...
    default void visitLambda(Tree.Lambda expr, FuncVisitor mv) {
        var capVars = expr.scope.capVars();

        var originalFunc = mv.freshLabel();
        mv.visitBranch(originalFunc);
        int num = capVars.size();
        lambdaStack.push(expr);
        FuncVisitor lambdaMv = mv.visitLambdaFunc(expr.pos, num + expr.params.size());
        for (var param : expr.params) {
            param.symbol.temp = lambdaMv.getArgTemp(num);
            ++num;
        }
        if (expr.withBody()) {
            expr.body.accept(this, lambdaMv);
        } else {
            expr.expr.accept(this, lambdaMv);
            lambdaMv.visitReturn(expr.expr.val);
        }
        lambdaMv.visitEnd();
        lambdaStack.pop();
        mv.visitLabel(originalFunc);

        var tmp = mv.visitIntrinsicCall(Intrinsic.ALLOCATE, true, mv.visitLoad(12 + 4 * capVars.size()));
        mv.visitStoreTo(tmp, mv.visitLoad(2));
        mv.visitStoreTo(tmp, 4, mv.visitFuncEntry(expr.pos));
        mv.visitStoreTo(tmp, 8, mv.visitLoad(capVars.size()));
        for (int i = 0; i < capVars.size(); i++ )
            mv.visitStoreTo(tmp, 12 + i * 4, getCapturedVar(capVars.get(i).name, mv).orElse(capVars.get(i).temp));
        expr.val = tmp;
    }
    ...
    private Optional<Temp> getCapturedVar(List<VarSymbol> capVars, String name, FuncVisitor mv) {
        for (int i = 0; i < capVars.size(); i++) {
            if (capVars.get(i).name.equals(name)) {
    ...
    private Optional<Temp> getCapturedVar(String name, FuncVisitor mv) {
        if (lambdaStack.empty()) {
            ...
            return getCapturedVar(lambdaStack.peek().scope.capVars(), name, mv);
        }
    }
}
