...
public class Typer extends Phase<Tree.TopLevel, Tree.TopLevel> implements TypeLitVisited {
    ...
    public void visitVarSel(Tree.VarSel expr, ScopeStack ctx) {
                ...
                if (symbol.get().isVarSymbol()) {
                    ...
                    if (var.isMemberVar()) {
                        if (ctx.currentMethod().isStatic()) {
                            ...
                            expr.setThis();
                            capVar = (VarSymbol) ctx.currentMethod().scope.find("this").get();
                        }
                    }
                    if (lambdaLevel > 0 && capVar.domain() != ctx.currentScope()) {
                        Scope currentLambdaScope = ctx.currentScope();
                        while (capVar.domain() != currentLambdaScope) {
                            if (currentLambdaScope.isLocalScope()) {
                                ...
                                ((LambdaScope) currentLambdaScope).capture(capVar);
                    ...
                    expr.isStatic = method.isStatic();
                    if (!ctx.currentMethod().isStatic()) {
                        expr.setThis();
                    }
                    expr.calleeName = ctx.currentClass().name;
        ...
        if (field.isPresent() && field.get().isVarSymbol()) {
            ...
            expr.isMethodName = true;
            expr.isStatic = ((MethodSymbol) field.get()).isStatic();
